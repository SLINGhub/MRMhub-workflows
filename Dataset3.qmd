...
The data come from a human plasma lipidomics study aimed at identifying plasma lipid biomarkers of rapid renal function decline in patients with type 2 diabetes, part of the Diabetes StudY in Nephropathy And other Microvascular cOmplications (DYNAMO) project (Chen *et al.* [-@chen2025]). The lipidomics analysis followed the method of Huynh *et al.* with minor modifications, using the same QC sample types as Dataset 1. This dataset covers the largest of the study's four patient cohorts: raw MS data for samples collected at the National University Hospital, Singapore, comprising 3,816 plasma samples and 775 blank and QC samples (4,591 in total). In total, 748 MRM transitions were monitored, from which 828 features were integrated. For the biological and clinical conclusions related to these data, we refer readers to the original publication [@chen2025].
...
`mrmhub-workflows.zip` is the complete workflows repository *including* the Dataset 3 data under `data/dataset-3/`. Unzip it, open the project and run `quarto render Dataset3.qmd` — nothing else is needed to reproduce this notebook.

`MRMhub-Dataset3.zip` in the same record holds the raw mzML files together with the MRMhub-INTEGRATOR application and all of its input files, so the peak integration described below can be re-run as published or with new parameters. It is **not** required here: this notebook starts from INTEGRATOR's `long.csv` output, which is already included in the repository.

Note that the Dataset 3 data exceed GitHub's file-size limit and are therefore not part of the [GitHub repository](https://github.com/SLINGhub/MRMhub-workflows) — use the Zenodo archive to reproduce this workflow.
:::

# Raw Data Processing: Peak Picking and Integration

This section describes key aspects of raw data processing workflow used for this dataset. For more a detailed description on the workflow, see [Workflow for Dataset 1](https://slinghub.github.io/MRMhub-workflows/Dataset1.html) and the [INTEGRATOR Manual](https://slinghub.github.io/MRMhub/integrator/manual.html).
...
The results from the peak integration performed with the INTEGRATOR workflow, as described in the previous section, were imported into a `MRMhubExperiment` data object. This object represents the central data container used in this postprocessing workflow. Note that the original result file from INTEGRATOR ('long.csv') has been renamed to 'Dataset3_MRMhub-INTEGRATOR_Final.csv'.
...
plot_responsecurves(
  ...
  output_pdf = FALSE,
  path = "response-curves-dataset3.pdf",

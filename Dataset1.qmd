...
# This site was rendered with mrmhub 0.9.3 (the QUANT module of MRMhub). This
...
::: callout-note
## mrmhub version
These notebooks were rendered with **mrmhub 0.9.3** (QUANT module of
...
fig4d <- plot_feature_correlations(
  ...
  cor_min = 0.1,sort_by_corr = TRUE, return_plots = TRUE,show_progress = F,
...
We now went back to the MRMhub-INTEGRATOR workflow and corrected the peak integration errors The corrected data is available in the file `Dataset1_MRMhub-INTEGRATOR.csv`. We import the corrected data, which we will use from now.
...
extfig3a <- plot_rt_vs_chain(mexp_initial, qc_types = "SPL", x_var = "total_c", font_base_size = 6)+
  theme( 
      legend.position = "right",
      legend.direction = "vertical", 
      legend.text = element_text(size = base_font_size*0.8), 
...
extfig3b <- plot_rt_vs_chain(mexp, qc_types = "SPL", x_var = "total_c", font_base_size = 6)+
  theme( 
      legend.position = "right",
      legend.direction = "vertical", 
      legend.text = element_text(size = base_font_size*0.8), 
...
fig5c <- plot_runscatter(mexp, variable = "conc_raw", 
                ...
                show_progress = FALSE,
                output_pdf = FALSE,
                path = "rt_all.pdf", return_plots = TRUE)[[1]]+
  theme(
    legend.position = "inside",
    legend.direction = "horizontal", 
    legend.text = element_text(size = base_font_size*0.8), 
...
plot_runscatter(mexp, 
  ...
  output_pdf = TRUE,
  path = "output/suppl-fig1_runscatter_area.pdf", return_plots = FALSE)
...
plot_runscatter(mexp, 
  ...
  multithreading = TRUE,   # parallelise PDF page generation across the mirai daemons
  output_pdf = TRUE,
  path = "output/suppl-fig2_runscatter_conc-raw.pdf", return_plots = FALSE)
...
plot_runscatter(mexp, 
  ...
  multithreading = TRUE,   # parallelise PDF page generation across the mirai daemons
  output_pdf = TRUE,
  path = "output/suppl-fig3_runscatter_conc-final.pdf", return_plots = FALSE)
```

...
# Options specific to rendering this document
...
Note that some plots are assigned to a variable (e.g. `fig4a`, `fig4b`) so they can be reused at the end of this workflow to assemble the multi-panel manuscript figures.

::: callout-important
## Data availability

All code and data for this workflow are deposited in the Zenodo record [10.5281/zenodo.15370293](https://doi.org/10.5281/zenodo.15370293) (*MRMhub-workflows*). The corresponding MS raw data (mzML files), together with the MRMhub-INTEGRATOR application and its input files, are deposited as `MRMhub-Dataset1.zip` in the same record.
:::

# Raw Data Processing: Peak Picking and Integration

This section describes key aspects of the raw data processing workflow, i.e. peak picking and peak integration, used for this dataset. For more details on the usage of the INTEGRATOR application, see the [INTEGRATOR Manual](https://slinghub.github.io/MRMhub/integrator/manual.html).
...
![MRMhub-INTEGRATOR application.](images/ds3-paste-3.png){#fig-ds1-integrator-app fig-align="center" width="62%"}
...
These notebooks were rendered with **mrmhub 0.9.9** (QUANT module of [MRMhub](https://github.com/SLINGhub/MRMhub), `main` branch); install with `pak::pak("SLINGhub/MRMhub")`.
:::
...
A zoom-in reveals that the endogenous (non-ISTD) features of one QC sample type, the LTRs, have on average approximately half the intensity of the other samples.
...
#| fig-format: png
#| dpi: 600
# An outlier sample, discussed later, is excluded here for demonstration
# purposes.
...
A further, detailed, batch-by-batch inspection revealed that in batch 6, the sample "Longit_batch6_51" at position 464 had markedly lower intensities of non-ISTD features, comparable to those of the blanks.
...
However, the ISTD signals were comparable with other samples. This suggests that this study sample was not added to the extraction vial before extraction. The sample is excluded from further analysis.
...
After metadata import, a summary of the data/metadata integrity validation is returned, highlighting for example duplicate or missing IDs. Warnings can be ignored (`ignore_warnings = TRUE`) to proceed with the analysis; errors, such as missing or duplicate IDs, must always be fixed before analysis can proceed.
...
SMs, on the other hand, show similar variability in BQCs and TQCs, suggesting that sample processing contributed less to the overall variability for this class.

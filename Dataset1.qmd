...
The data come from the SPERFECT study (*SPhingolipids as sErial biomaRkers oF disease progression in acutE myoCardial infarcTion*; Tan *et al.* [-@tan2022]), a targeted lipidomics analysis of plasma from 84 participants at risk of coronary artery disease. Participants contributed to a longitudinal arm with blood collection at up to five follow-up time points (378 samples), and a subset to a diurnal rhythm study (299 samples). Including QC samples, the dataset comprises 937 injections acquired by LC-MRM in six analytical batches; 464 MRM transitions were monitored, from which 482 features were integrated (a transition can yield multiple features, e.g. for isomers or isobars). The QC samples are solvent blanks (SBLK), process blanks (PBLK, containing the ISTD mix), technical QC (TQC, pooled aliquots of extracted study samples), batch QC (BQC, aliquots of pooled plasma extracted alongside the study samples) and response-curve QC (RQC, the TQC injected at different volumes). PBLKs and BQCs were co-extracted; PBLK, TQC and BQC samples were measured at regular intervals throughout the analysis to monitor background signal, instrument performance and overall process performance, respectively.

This example workflow demonstrates the complete MRMhub data processing pipeline on this dataset — peak integration, quantitation, quality control and reporting. It covers many elements of the MRMhub workflow for demonstration purposes, some of which may not be applicable or necessary when processing other datasets.

::: callout-important
## Data availability

All code and data for this workflow are deposited in the Zenodo record [10.5281/zenodo.15370293](https://doi.org/10.5281/zenodo.15370293) (*MRMhub-data*).

`mrmhub-workflows.zip` is the complete workflows repository *including* the Dataset 1 data under `data/dataset-1/`. Unzip it, open the project and run `quarto render Dataset1.qmd` — nothing else is needed to reproduce this notebook.

`MRMhub-Dataset1.zip` in the same record holds the raw mzML files together with the MRMhub-INTEGRATOR application and all of its input files, so the peak integration described below can be re-run as published or with new parameters. It is **not** required here: this notebook starts from INTEGRATOR's `long.csv` output, which is already included in the repository.

Note that the Dataset 1 data exceed GitHub's file-size limit and are therefore not part of the [GitHub repository](https://github.com/SLINGhub/MRMhub-workflows) — use the Zenodo archive to reproduce this workflow.
:::
...
# installed from the `main` branch.
pak::pak("SLINGhub/MRMhub@main")

# Parallel processing (mirai) and coloured console output (fansi)
pak::pak(c("mirai", "fansi"))
```
...
if (knitr::is_latex_output()) options(cli.num_colors = 1, crayon.enabled = FALSE) else mrmhub_enable_cli_color()
```
...
save_plot(
  extfig2, "output/ExtendedFig-4_RLAs.pdf",
...
save_plot(
  extfig3, "output/ExtendedFig-5.png",
  width = 180, height = 250)
```
...
save_plot(
  extfig4, "output/ExtendedFig-6_PCAloading", format = c("pdf", "png"),
  width = 180, height = 250)
```
...
save_plot(
  extfig5[[1]], "output/ExtendedData_Fig7_ISTDtrends.png",
...
extfig6 <- extfig6a /
  ...
  theme(
    ...
    strip.placement = 'inside')
save_plot(
  extfig6, "output/ExtendedData_Fig8_CVnormbeforeafter.pdf",

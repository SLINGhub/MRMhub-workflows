...
memory benchmark) — using the **Dataset 1 (SPERFECT)** pipeline, on a uniform 3 × 2 grid
...
pD2 <- plot_abundanceprofile(
  ...
  theme(axis.text.y = element_text(size = base_font_size * 0.6),
        axis.text.y.right = element_text(size = base_font_size * 0.6),
        legend.text = element_text(size = base_font_size * 0.7),
        legend.key.size = unit(base_font_size * 0.7, "pt"))
pD2
```
...
before rendering; the LC-MS-acquisition and peak-integration rows are kept as reference
estimates (acquisition is instrument time, integration runs in the separate INTEGRATOR app).
...
bx0 <- 0.3; bx1 <- 4.3; bh <- 0.72; xc_s <- 5.8; xc_d <- 9.2; arrow_col <- "#b98a2e"

pE <- ggplot() +
  geom_rect(data = boxes, aes(xmin = bx0, xmax = bx1, ymin = y - bh/2, ymax = y + bh/2),
            fill = boxes$fill, colour = boxes$col, linewidth = 0.7) +
  geom_text(data = boxes, aes(x = (bx0+bx1)/2, y = y, label = stage),
            size = e_font*0.92/.pt, lineheight = 0.9) +
  annotate("segment", x = (bx0+bx1)/2, xend = (bx0+bx1)/2, y = c(4,3,2) - bh/2,
           yend = c(3,2,1) + bh/2, colour = arrow_col, linewidth = 2.4,
           arrow = arrow(type = "closed", length = unit(5, "pt"))) +
  annotate("text", x = xc_s, y = 4.92, label = "SPERFECT", fontface = "bold.italic", size = e_font*0.75/.pt) +
  annotate("text", x = xc_d, y = 4.92, label = "DYNAMO",   fontface = "bold.italic", size = e_font*0.75/.pt) +
  annotate("text", x = xc_s, y = 4.6,  label = "(n=937)",  fontface = "italic", size = e_font*0.7/.pt) +
  annotate("text", x = xc_d, y = 4.6,  label = "(n=4591)", fontface = "italic", size = e_font*0.7/.pt) +
  annotate("text", x = xc_s, y = 4, label = "2 weeks", size = e_font/.pt) +
  annotate("text", x = xc_d, y = 4, label = "8 weeks", size = e_font/.pt) +
  annotate("text", x = xc_s, y = 3, label = "8 sec",   size = e_font/.pt) +
  annotate("text", x = xc_d, y = 3, label = "42 sec",  size = e_font/.pt) +
  ...
  annotate("text", x = bx1,  y = 0.28, label = "Max. memory footprint", hjust = 1,
           fontface = "italic", size = e_font*0.9/.pt) +
  annotate("text", x = xc_s, y = 0.28, label = mem1, size = e_font/.pt, fontface = "bold") +
  annotate("text", x = xc_d, y = 0.28, label = mem3, size = e_font/.pt, fontface = "bold") +
  annotate("text", x = (bx0+xc_d)/2, y = -0.2, label = "Apple MacBook Pro, M2 Pro, 32 Gb RAM",
           colour = "#c0392b", fontface = "italic", size = e_font*0.72/.pt) +
  scale_x_continuous(limits = c(-1.7, xc_d + 1.5)) +
  scale_y_continuous(limits = c(-0.45, 5.25)) +
  coord_cartesian(clip = "off") + theme_void() + theme(plot.margin = margin(4, 8, 4, 4))
pE
```
...
reporting plots) and panel **e** (the runtime benchmark), on a uniform 3 × 2 grid. Panels **a**
...
```{r chunk12-assemble, fig.width = 7.2, fig.height = 5.5}
...
ggsave("output/ManuscriptFigure_panelsCDE.png", figCDE,
       width = 182, height = 140, units = "mm", dpi = 300, device = ragg::agg_png)
ggsave("output/ManuscriptFigure_panelsCDE.pdf", figCDE,
       width = 182, height = 140, units = "mm", dpi = 300, device = cairo_pdf)

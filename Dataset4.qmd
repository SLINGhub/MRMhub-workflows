...
spl_area_excl <- mrm_q_area |>
  ...
  mutate(
    excl_mrm = !is.na(area_mrm) &
      !is.na(cala_area_mrm) &
      area_mrm < cala_area_mrm, excl_mh = !is.na(area_mh) &
      !is.na(cala_area_mh) &
      area_mh < cala_area_mh) |>
  select(
    analysis_id,
    analyte_id, area_mrm, area_mh, cala_area_mrm, cala_area_mh, excl_mrm,
    excl_mh)
```
...
save_fig <- function(plot, stem, height, width = 180) {
  ggsave(
    ...
    plot, device = cairo_pdf, width = width, height = height, units = "mm")
  ggsave(
    file.path("output", paste0(stem, ".png")),
    plot, device = ragg::agg_png, width = width, height = height,
    units = "mm", dpi = 300)
  invisible(plot)
}
```
...
chrom_candidates <- d_peakcmp_reint |>
  ...
  transmute(
    feature_id,
    analysis_id, qc_type, area_mrm = round(area_mrm),
...
EQA samples). The reference concentrations are read from the metadata workbook's
...
#| fig-cap: "**EQA accuracy against SKML reference values.** Per-analyte percent bias against the SKML reference concentrations (±15 % dashed), MRMhub vs MassHunter, one point per EQA sample per method (DHEA and DHEAS excluded). Panel D of @fig-d4-fig2."
#| fig-width: 7
#| fig-height: 4.5
...
d_acc <- d_conc_raw |>
  ...
  left_join(
    ...
    by = c("analysis_id", "analyte_id")) |>
  mutate(
    conc_mrm = if_else(coalesce(excl_mrm, FALSE), NA_real_, conc_mrm),
    conc_mh = if_else(coalesce(excl_mh, FALSE), NA_real_, conc_mh)) |>
...
p_eqa <- d_acc |>
  ...
  pivot_longer(
    c(bias_mrmhub, bias_masshunter),
    names_to = "method", values_to = "bias") |>
  filter(!is.na(bias)) |>
  mutate(
    method = recode(
      method,
      bias_mrmhub = "MRMhub", bias_masshunter = "MassHunter"),
    method = factor(method, levels = c("MRMhub", "MassHunter")),
    sample_lbl = factor(sample_lbl, levels = x_levels)) |>
  ggplot(aes(sample_lbl, bias, colour = method, shape = method)) +
  ...
  scale_shape_manual(
    values = c(MRMhub = 16, MassHunter = 17),
    name = "Method") +
...
#| label: fig-d4-fig2
#| fig-cap: "**MRMhub vs MassHunter — quantification.** Composite of the four panels above; reproduced as the manuscript's **Figure 2**. **(A)** Calibration-fit comparison, **(B)** concentration equivalence on identical areas, **(C)** full end-to-end agreement, **(D)** EQA accuracy. Exported to `output/fig2_quantification_comparison.{pdf,png}` (180 mm)."
#| fig-width: 7.1
#| fig-height: 9.3
...
against the SKML EQA reference values (@fig-d4-fig2). Any residual differences are
